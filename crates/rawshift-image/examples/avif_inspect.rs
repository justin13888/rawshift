//! AVIF inspection example.
//!
//! Enumerates auxiliary images (thumbnails, alpha plane, depth maps) and
//! dumps all extracted metadata — all of which works with **no hardware
//! decoder** — then attempts pixel decode, which needs a hardware AV1
//! backend (`hw` feature + a usable platform decoder) and otherwise reports
//! `HwDecoderUnavailable`.
//!
//! Usage:
//!   cargo run --example avif_inspect --features "avif hw" -- <input.avif>
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let path = std::env::args()
        .nth(1)
        .expect("Usage: avif_inspect <input.avif>");
    let data = std::fs::read(&path)?;

    let file = AvifFile::open(data)?;
    ...
    // ── Pixel decode (needs a hardware AV1 decoder) ──────────────────────────
    println!(
        "\nHardware AV1 decode available: {}",
